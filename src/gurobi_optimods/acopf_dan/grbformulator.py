        ...
        if branch.status:
            if alldata["substitute_nonconv"] == False or alldata["use_ef"] == False:
                ...
                branch.Pdeffconstr = model.addConstr(
                    expr == Pvar_f[branch], name=branch.Pfcname
                )
                ...
                branch.Pdeftconstr = model.addConstr(
                    expr == Pvar_t[branch], name=branch.Ptcname
                )
                count += 2
        else:
            branch.Pdeffconstr = model.addConstr(
        ...
        if branch.status:
            if alldata["substitute_nonconv"] == False or alldata["use_ef"] == False:
                ...
                branch.Qdeffconstr = model.addConstr(
                    expr == Qvar_f[branch], name=branch.Qfcname
                )
                ...
                branch.Qdeftconstr = model.addConstr(
                    expr == Qvar_t[branch], name=branch.Qtcname
                )
                count += 2
